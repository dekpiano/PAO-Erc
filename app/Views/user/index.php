...
<?= $this->section('content') ?>
    ...
    <section id="scholarships" class="py-24 bg-slate-50 relative overflow-hidden">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row justify-between items-end mb-12 gap-6" data-aos="fade-up">
                <div>
                    <span class="text-amber-600 font-black tracking-widest uppercase text-xs mb-3 inline-block">ทุนการศึกษา</span>
                    <h3 class="text-4xl font-black text-slate-900 leading-tight">โอกาสเพื่ออนาคตทางการศึกษา</h3>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <?php if(!empty($latest_scholarships)): ?>
                    <?php foreach($latest_scholarships as $sch): ?>
                        <div class="group bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500" data-aos="fade-up">
                            <div class="aspect-[4/3] bg-slate-100 relative overflow-hidden">
                                <?php if($sch['sch_cover']): ?>
                                    <img src="<?= base_url('uploads/scholarships/covers/' . $sch['sch_cover']) ?>" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                                <?php else: ?>
                                    <div class="w-full h-full flex items-center justify-center text-slate-300">
                                ...
                                <div class="absolute top-4 left-4">
                                    <span class="px-3 py-1 bg-amber-500 text-white text-[10px] font-black rounded-lg uppercase tracking-wider shadow-lg">
                                        <?= $sch['sch_amount'] ?: 'ทุนการศึกษา' ?>
                                    </span>
                                </div>
                            </div>
                            <div class="p-6">
                                <h4 class="text-lg font-black text-slate-900 group-hover:text-amber-600 transition-colors leading-tight mb-4 line-clamp-2 min-h-[3rem]">
                                    <?= $sch['sch_title'] ?>
                                </h4>
                                <div class="flex items-center justify-between pt-4 border-t border-slate-50">
                                    <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                        <?php if($sch['sch_deadline']): ?>
                                            <i data-lucide="clock" class="w-3.5 h-3.5 inline mr-1"></i> หมดเขต: <?= date('d/m/Y', strtotime($sch['sch_deadline'])) ?>
                                        <?php else: ?>
                                            <i data-lucide="check-circle" class="w-3.5 h-3.5 inline mr-1 text-emerald-500"></i> เปิดรับสมัคร
                                        <?php endif; ?>
                                    </div>
                                    <a href="<?= base_url('booking/' . $sch['sch_slug']) ?>" class="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-amber-100 group-hover:text-amber-600 transition-all" title="จองคิว">
                ...
                <?php else: ?>
                    <div class="col-span-full py-12 text-center bg-white rounded-3xl border border-slate-100">
                        <p class="text-slate-400 font-bold">ยังไม่มีข้อมูลทุนการศึกษาในขณะนี้</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>

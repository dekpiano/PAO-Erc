...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 sm:gap-4 mb-6 w-full">
    ...
    <div class="flex flex-wrap gap-3 w-full md:w-auto">
        <a href="<?= base_url('science-week/staff?' . ($_SERVER['QUERY_STRING'] ?? '')) . '#reg-row-' . $reg['reg_id'] ?>" class="w-full md:w-auto justify-center px-4 py-3 rounded-2xl bg-slate-900 border-2 border-slate-700 text-slate-200 hover:text-white font-extrabold text-xs sm:text-sm hover:bg-slate-800 transition-colors flex items-center gap-2 shadow-sm">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="glass-card p-6 sm:p-8 rounded-3xl border border-slate-800 shadow-xl">
            ...
            <form id="editRegForm" action="<?= base_url('science-week/staff/update/' . $reg['reg_id'] . '?' . ($_SERVER['QUERY_STRING'] ?? '')) ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                <?= csrf_field() ?>
                ...
                <div class="flex gap-4 pt-4">
                    <a href="<?= base_url('science-week/staff?' . ($_SERVER['QUERY_STRING'] ?? '')) . '#reg-row-' . $reg['reg_id'] ?>" class="flex-1 py-3.5 border-2 border-slate-700 text-slate-300 font-extrabold text-sm hover:bg-slate-800 rounded-2xl transition-colors flex items-center justify-center gap-1">
                        ยกเลิก
                    </a>
